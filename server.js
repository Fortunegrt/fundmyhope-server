...
//
// Uses Paystack for donations (server holds the Paystack SECRET key;
// the frontend only ever sees the PUBLIC key, embedded in the HTML).
// Withdrawals are manual — this server just emails the team via Resend
// when an organizer requests one; no money moves automatically.
...
if (!process.env.PAYSTACK_SECRET_KEY) {
  console.error('Missing PAYSTACK_SECRET_KEY environment variable. Set it in Render → Environment.');
  process.exit(1);
}
